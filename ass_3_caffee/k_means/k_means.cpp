...
int main (int argc, char** argv)
{
    ...
    caffe_gpu_powx<Dtype>(train_count, train_data->gpu_data(), 2, blob_train_out->mutable_gpu_data());
    ...
    caffe_gpu_gemm<Dtype>(CUBLAS_OP_N, CUBLAS_OP_N, rows, rows2, rows, 1, blob_train_out->gpu_data(), unary_blob->gpu_data(), 0, result_blob->mutable_gpu_data());
    Blob<Dtype>* temp_blob = new Blob<Dtype>(1, 1, rows, rows2);
    caffe_gpu_gemm<Dtype>(CUBLAS_OP_N, CUBLAS_OP_N, rows, rows2, rows2, 1, unary_blob->gpu_data(), blob_test_out->gpu_data(), 0, temp_blob->mutable_gpu_data());
    ...
    caffe_gpu_gemm<Dtype>(CUBLAS_OP_N, CUBLAS_OP_T, rows, rows2, cols, 2, train_data->gpu_data(), test_data->gpu_data(), 0, temp_blob->mutable_gpu_data());
    // find final D
    ...
    for(int c = 0; c < rows2 ; c++){
      ...
      count << "class of the vectors is " << classes[minimal_index] << "\n";
