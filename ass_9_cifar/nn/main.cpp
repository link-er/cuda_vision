...
int main(int argc, char** argv) {
    ...
    caffe_powx<Dtype>(blob_data->count(), blob_data->gpu_data(), 2.0, blob_train_out->mutable_gpu_data());

    Blob<Dtype>* blob_test_out = new Blob<Dtype>(1, 1, m, d);
    caffe_powx<Dtype>(blob_test_data->count(), blob_test_data->gpu_data(), 2.0, blob_test_out->mutable_gpu_data());
    ...
    caffe_gemm<Dtype>(CblasTrans, CblasTrans, m, n, d, 1., unary_blob1->gpu_data(), blob_train_out->gpu_data(),
    ...
    caffe_gemm<Dtype>(CblasNoTrans, CblasTrans, m, n, d, 1, blob_test_out->gpu_data(), unary_blob2->gpu_data(),
                          1, result_blob->mutable_gpu_data());
    delete unary_blob2;
    delete blob_test_out;
    cout << "Multiplied by unary and added the first one multiplied\n";

    caffe_gemm<Dtype>(CblasNoTrans, CblasTrans, m, n, d, -2., blob_test_data->gpu_data(),
